...
server.tool("show-notification", "Show a system notification using the default notifier for the current platform", commonNotificationSchema, async (params) => {
    ...
    return new Promise((resolve) => {
        notifier.notify(params, (err, response, metadata) => {
            if (err) {
                resolve({
                    content: [{ type: "text", text: `Error showing notification: ${err.message}` }],
                });
            }
            else {
                resolve({
                    content: [
                        { type: "text", text: "Notification shown successfully" },
                        ...(metadata ? [{ type: "text", text: `Metadata: ${JSON.stringify(metadata)}` }] : []),
                        ...(response ? [{ type: "text", text: `Response: ${JSON.stringify(response)}` }] : [])
                    ],
                });
            }
        });
...
server.tool("show-notification-macos", "Show a notification using macOS Notification Center", commonNotificationSchema, async (params) => {
    return new Promise((resolve) => {
        ...
        notifier.notify(params, (err, response, metadata) => {
            if (err) {
                ...
                resolve({
                    content: [
                        { type: "text", text: "macOS notification shown successfully" },
                        ...(metadata ? [{ type: "text", text: `Metadata: ${JSON.stringify(metadata)}` }] : []),
                        ...(response ? [{ type: "text", text: `Response: ${JSON.stringify(response)}` }] : [])
                    ],
                });
...
server.tool("show-notification-linux", "Show a notification using Linux notify-send", {
    ...
    return new Promise((resolve) => {
        ...
        notifier.notify(params, (err) => {
            if (err) {
                ...
                resolve({
                    content: [{ type: "text", text: "Linux notification shown successfully" }],
                });
            }
        });
    });
});
server.tool("show-notification-windows-toast", "Show a notification using Windows Toast notifications (Windows 8+)", {
    ...commonNotificationSchema,
    ...
    return new Promise((resolve) => {
        ...
        notifier.notify(params, (err) => {
            if (err) {
                ...
                resolve({
                    content: [{ type: "text", text: "Windows Toast notification shown successfully" }],
                });
            }
        });
    });
});
server.tool("show-notification-windows-balloon", "Show a notification using Windows Balloon notifications (Windows 7 and earlier)", {
    ...commonNotificationSchema,
    ...
    return new Promise((resolve) => {
        ...
        notifier.notify(params, (err) => {
            if (err) {
                ...
                resolve({
                    content: [{ type: "text", text: "Windows Balloon notification shown successfully" }],
                });
            }
        });
    });
});
server.tool("show-notification-growl", "Show a notification using Growl", {
    ...commonNotificationSchema,
    ...
    return new Promise((resolve) => {
        ...
        notifier.notify(params, (err) => {
            if (err) {
                ...
                resolve({
                    content: [{ type: "text", text: "Growl notification shown successfully" }],
                });
            }
        });
    });
